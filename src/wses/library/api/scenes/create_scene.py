...
from wses.commands import get_project_id
from wses.library.api.auth import send_auth_request
from wses.library.api.crud import get_status_values
...
def build_body(args, project_id):
    ...
    if args.status:
        status_id = [st["id"] for st in statuses if st["name"] == args.status][0]
