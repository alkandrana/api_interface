...
from .sync import get_projects_from_log, sync_projects, get_local_project_details, get_unsaved_projects
...
def create_project_interactive(code):
    payload = {"code": code, "title": input("Book title: "), "goal": input("Target word count (optional): ")}
    if not payload["goal"]:
        payload["goal"] = 100000
    request = {
        "method": "POST",
...
def batch_projects(args):
    if args.source == "api":
        ...
        if check_server_health(url):
            ...
            print("Server is down, or config is out of date. Update it with 'wlogs config api'.")
            sys.exit(1)
